...
title: <!--html_preserve--><table width='100%'><tr><td><!--/html_preserve-->CONFIDENTIAL-NOT MENTION- NOT SPREAD<!--html_preserve--></td><td align='right'><!--/html_preserve-->2025_01TFMLBlanc- Null Scenario Versió 5 <!--html_preserve--></td></tr></table> <!--/html_preserve-->
...
format: 
  html:
    output-file: 'Analysis_2025_01TFMLBlanc_null_v5.html'
...
# Normal distribution: Null scenario
...
# Dades simulades
dades_n <- data.frame(y, x)


# Guardar dataframes
# dades <- list()
# save(dades, file = "dades_loop_null_v2.Rdata")

load("dades_loop_null_v2.Rdata")

var_seed <- params$report_seed

dades <- dades |> purrr::list_assign(var_seed = dades_n)
...
#head(dades_n)
#tapply(dades_n$y, dades_n$x, mean)
#tapply(dades_n$y, dades_n$x, sd)
...
`r tab_cap("Description of data simulation: Null scenario")`
...
dades_n$x_c<-factor(dades_n$x,
                    levels = c(0,1),
                    labels = c("Non-disease", "Disease"))

dades_n$y_pred<-dades_n$y

d <- dplyr::select(dades_n,c(y_pred, x_c, x))
d <- filter(d, x_c%in%c("Non-disease", "Disease"))
d$x_c<-factor(d$x_c, levels = c("Non-disease", "Disease"))


#png(r"(I:\CTebe\2_Projectes\2025_01TFMLBlanc\4_Productes\Presentation\WWW\null_density.png)",width = 5000,height = 4000,res = 600)
...
#png(r"(I:\CTebe\2_Projectes\2025_01TFMLBlanc\4_Productes\Presentation\WWW\null_roc.png)",width = 5000,height = 4000,res = 600)


myroc <- roc(d$x, d$y_pred, ci=TRUE, direction="<")
plot(myroc, main="ROC curve and AUC for null scenario", xlim = c(1, 0), lwd=3)
...
# load(file="I:/CTebe/2_Projectes/2025_01TFMLBlanc/2_Dades/2_Analisi/Null/Null_scenario.Rda")
...
kable(tab, caption = "Contingency Table - Euclidean", align = "c") %>%
   kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                 full_width = F, position = "center")
...
kable(tab, caption = "Contingency Table - Misclassification cost", align = "c") %>%
   kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                 full_width = F, position = "center")
...
metrics_misscost <- data.frame(
  ...
  optimal_cutpoint = cutoff_misscost[1],
...
metrics_t<-metrics_misscost %>%
   select(method, optimal_cutpoint, sensitivity, sensitivity_low, sensitivity_high,
          specificity, specificity_low, specificity_high, ppv, ppv_low, ppv_high, npv, npv_low, npv_high, accuracy, accuracy_low, accuracy_high) %>%
   gt() %>%
   tab_header(title = "Optimal Cut-off Point Metrics - Misclassification cost") %>%
   fmt_number(
     columns = where(is.numeric),
     decimals = 3
   )
...
# save(method_df, file = "method_df_null_v2.Rdata")

load("method_df_null_v2.Rdata")
