...
title: <!--html_preserve--><table width='100%'><tr><td><!--/html_preserve-->CONFIDENTIAL-NOT MENTION- NOT SPREAD<!--html_preserve--></td><td align='right'><!--/html_preserve-->2025_01TFMLBlanc- Moderate Scenario - Results Versió 2 <!--html_preserve--></td></tr></table> <!--/html_preserve-->
...
format: 
  html:
    output-file: 'Analysis_2025_01TFMLBlanc_moderate_results_v2.html'
...
#Read data
load("I:/CTebe/2_Projectes/2025_01TFMLBlanc/3_CodiR/dades_loop_moderate_v2.Rdata") 

#Read cutoff
load("I:/CTebe/2_Projectes/2025_01TFMLBlanc/3_CodiR/method_df_moderate_v2.Rdata")

```


# Results for Moderate scenario
...
# Metrics [CI]
tab_cutoffs_formatted_m <- tab_cutoffs_m |>
  mutate(
...
#save(tab_cutoffs_formatted_m, file = file.path("I:/CTebe/2_Projectes/2025_01TFMLBlanc/2_Dades/2_Analisi/Moderate", "tab_cutoffs_moderate.Rda"))
     
# Final comparison table
library(gt)

tab_cutoffs_formatted_m |>
  gt()|>
  tab_header(
    title = md("**Comparison of Cut-off Methods: Moderate scenario**")
  ) |>
  cols_label(
    Method = "Method",
...
tab_cutoffs_m$method <- factor(tab_cutoffs_m$method, levels = c(
  "log(DOR)",
  "Misclassification cost",
  "Cost approach",
  "Index of Union",
...
tab_cutoffs_cut_m <- tab_cutoffs_m %>%
  mutate(cutoff_label = sprintf("%.2f (%.2f–%.2f)", optimal_cutpoint, optimal_cutpoint_low, optimal_cutpoint_high))

save(tab_cutoffs_cut_m, file = file.path("I:/CTebe/2_Projectes/2025_01TFMLBlanc/2_Dades/2_Analisi/Moderate", "tab_cutoffs_cut_m.Rda"))
   
ggplot(tab_cutoffs_cut_m, aes(x = optimal_cutpoint, y = method)) +
  ...
  labs(
     title = md("Cut-off Points by Method: Moderate scenario"),
...
ggplot(tab_cutoffs_se, aes(x = sensitivity, y = method)) +
  ...
  labs(
     title = "Sensitivity (Se) by Method: Moderate scenario",
    x = "Estimates for Se",
    y = "Method"
  ) +
...
ggplot(tab_cutoffs_sp, aes(x = specificity, y = method)) +
  ...
  labs(
     title = "Specificity (Sp) by Method: Moderate scenario",
    x = "Estimates for Sp",
    y = "Method"
  ) +
  theme_minimal(base_size = 13) + 
...
ggplot(tab_cutoffs_ppv, aes(x = ppv, y = method)) +
  ...
  labs(
     title = "Positive Predictive Value (PPV) by Method: Moderate scenario",
    x = "Estimates for PPV",
    y = "Method"
  ) +
...
ggplot(tab_cutoffs_npv, aes(x = npv, y = method)) +
  ...
  labs(
     title = "Negative Predictive Value (NPV) by Method: Moderate scenario",
    x = "Estimates for NPV",
    y = "Method"
  ) +
  theme_minimal(base_size = 13) + xlim(0,1.5)+ 
...
ggplot(tab_cutoffs_acc, aes(x = accuracy, y = method)) +
  ...
  labs(
    title = "Accuracy by Method: Moderate scenario",
