...
title: <!--html_preserve--><table width='100%'><tr><td><!--/html_preserve-->CONFIDENTIAL-NOT MENTION- NOT SPREAD<!--html_preserve--></td><td align='right'><!--/html_preserve-->2025_01TFMLBlanc- Moderate Scenario Versió 1 <!--html_preserve--></td></tr></table> <!--/html_preserve-->
...
format: 
  html:
    output-file: 'Analysis_2025_01TFMLBlanc_moderate_v1_loop_v2.html'
...
# Normal distribution: Moderate scenario
...
# dades <- list()
# save(dades, file = "dades_loop_moderate_v2.Rdata")

load("dades_loop_moderate_v2.Rdata")

var_seed <- params$report_seed

dades <- dades |> purrr::list_assign(var_seed = dades_em)
...
# tapply(dades_em$y, dades_em$x, mean)
# tapply(dades_em$y, dades_em$x, sd)

```



`r tab_cap("Description of data simulation: Moderate scenario")`
...
dades_em$x_c<-factor(dades_em$x,
                    levels = c(0,1),
                    labels = c("Non-disease", "Disease"))

dades_em$y_pred<-dades_em$y

d <- dplyr::select(dades_em,c(y_pred, x_c, x))
d <- filter(d, x_c%in%c("Non-disease", "Disease"))
d$x_c<-factor(d$x_c, levels = c("Non-disease", "Disease"))

#png(r"(I:\CTebe\2_Projectes\2025_01TFMLBlanc\4_Productes\Presentation\WWW\moderate_density.png)",width = 5000,height = 4000,res = 600)
...
ggplot(d, aes(y_pred,fill=x_c,color=x_c)) +
 ...
 theme_minimal()+labs(x="Predictor")+
  ggtitle("Density comparison for moderate scenario")+ 
...
#png(r"(I:\CTebe\2_Projectes\2025_01TFMLBlanc\4_Productes\Presentation\WWW\moderate_roc.png)",width = 5000,height = 4000,res = 600)

myroc <- roc(d$x, d$y_pred, ci=TRUE)
plot.roc(myroc, main="ROC curve and AUC for moderate scenario", xlim = c(1, 0), lwd=3)
...
# save(p, file = file.path("I:/CTebe/2_Projectes/2025_01TFMLBlanc/2_Dades/2_Analisi/Moderate", "Moderate_scenario.Rda"))
# 


# load(file="I:/CTebe/2_Projectes/2025_01TFMLBlanc/2_Dades/2_Analisi/Moderate/Moderate_scenario.Rda")
...
<!-- ```{r echo=FALSE, warning=FALSE, message=FALSE, cache=FALSE} -->
<!-- t_tot <- sum(table(d$x_c)[c("Disease")])/nrow(d) -->
<!-- #i el calculat amb la funció -->
<!-- k1 <- filter(d, x_c%in%c("Disease"))$y_pred -->
<!-- k2 <- filter(d, x_c%in%c("Non-disease"))$y_pred -->
<!-- rho<- t_tot -->
<!-- library(ThresholdROC) -->
<!-- t <- thres2(k1, k2, rho, na.rm=TRUE, method="equal", ci.method="boot", extra.info=TRUE) -->
<!-- cutoff <- t$T$thres -->
<!-- ``` -->

<!-- #### Cut-off and density function -->
<!-- ```{r echo=FALSE, warning=FALSE, message=FALSE, cache=FALSE} -->
<!-- x_outcome <-quantile(d$y_pred,probs = seq(0,1,0.01),na.rm=T) -->
...
<!-- for(i in x_outcome){ -->
<!--     d$y <- cut(d$y_pred, breaks = c(-Inf,i, Inf)) -->
<!--     if(all(table(d$y, d$x)>1)){ -->
<!--     m <- glm(x~y,data=d, family=poisson(link = "log")) -->
<!--     l <- summary(m) -->
<!--     pvalue <- c(pvalue, l$coefficients[2,4]) -->
<!--     ai<-c(ai,AIC(m)) -->
<!--     bi <- c(bi, BIC(m)) -->
...
<!-- d2 <- data.frame(val,ai,bi, hr, p1x,p2x,pvalue,br) -->
<!-- m <- min(d2$pvalue) -->
<!-- coffor <- d2[d2$pvalue==m, "val"][1] -->


<!-- g <- ggplot(d2,aes(x=val, y=hr))+ -->
<!--   geom_line()+ -->
<!--   geom_ribbon(aes(ymin = p1x, ymax =p2x), alpha = 0.1)+ -->
<!--   geom_hline(yintercept=0, col="red")+ -->
<!--   geom_vline(data=data.frame(x=cutoff),aes(xintercept=x, color="blue"),show_guide=TRUE)+ -->
<!--   geom_vline(data=data.frame(x=coffor),aes(xintercept=x, color="orange"),show_guide=TRUE)+ -->
<!--   xlab("Value of predictor")+ylab("RR logistic model") -->
<!-- g+scale_color_manual("Cut-off values", values=c(blue="blue",orange="orange"), -->
<!--                      labels=c("Youden Index", "RR", "Brier"))+ -->
<!--   theme_bw() -->
<!-- ``` -->
&nbsp; 
&nbsp; 
...
metrics_boot_maxdens <- data.frame(
  ...
  optimal_cutpoint = round(xintercept, 5),
  sensitivity = x["Sensitivity", "Estim."],
  sensitivity_low = x["Sensitivity", "Low.lim(95%)"],
  sensitivity_high = x["Sensitivity", "Up.lim(95%)"],
...
# Confusion matrix
d$y_cat <- ifelse(d$y_pred <= optimal_threshold, "Predicted non-disease", "Predicted disease")
...
(We chose the first cut-point because )
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
cutoff_misscost <- opt_misscost$cutoff[1]
...
`r tab_cap("Comparison of Cut-off Methods: Moderate scenario")`
...
# save(method_df, file = "method_df_moderate_v2.Rdata")

load("method_df_moderate_v2.Rdata")
